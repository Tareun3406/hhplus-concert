...
class ReservationRedisCacheRepository(
    ...
    fun increaseReservationCountToConcert(concert: Concert, referenceTime: LocalDateTime) {
        val variable = DateCalculator.truncatedDateTime(referenceTime, collectCycleMinute).toString()
        val rankedConcert = redissonClient.getScoredSortedSet<Concert>(RedisKeyType.RESERVATION_CONCERT_RANKING.getKeyWithVariable(variable))
        rankedConcert.addScore(concert, 1)
        rankedConcert.expire(Duration.ofMinutes(60)) // 캐시 유효시간 60분 (첫 적용 후 갱신되지 않음)
    }

    fun getRankedConcertList(rankingSize: Int, referenceTime: LocalDateTime): List<Concert> {
