...
class KafkaTest {
    ...
    @Test
    fun `JSON 문자열로 변환한된 메세지를 카프카에 전달하고 객체로 읽을 수 있다`() {
        // given
        ...
        val serialized: ByteArray = serializer.serialize("test-topic1", message)!! // 직렬화된 객체
        val serializedMessage = String(serialized) // DB에 저장할하고 카프카로 전송할 JSON 문자열

        // 카프카에 전송 및 읽기 시뮬레이션
//        val stringSerialized = StringSerializer().serialize("test-topic2", serializedMessage)!! // DB에 저장했던 JSON 문자열을 직렬화
//        val deserializer = JsonDeserializer(messageClass.java) // JSON 문자열을 직렬화 한 메세지를 객체로 역직렬화 가능한지 테스트
//        val deserializedMessage = deserializer.deserialize("test-topic2", serialized)

        // when
        kafkaTemplate.send("test-topic1", serializedMessage)
